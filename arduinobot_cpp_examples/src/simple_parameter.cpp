...
class SimpleParametr : public rclcpp::Node
{
    public:
        SimpleParametr() : Node("simple_parameter")
        {
            declare_parameter<int>("simple_int_param",28);
            declare_parameter<std::string>("simple_string_param", "Antonio");

            param_callbackhandle =  add_on_set_parameters_callback(std::bind(&SimpleParametr::paramChangeCallback, this, _1));

        }

    private:
        OnSetParametersCallbackHandle::SharedPtr param_callbackhandle;
...

{
    ...
    auto node = std::make_shared<SimpleParametr>();
    rclcpp::spin(node);
    rclcpp::shutdown();
